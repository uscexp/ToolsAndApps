...
public class ASTNegativeConstNode extends SimpleNode {
  public ASTNegativeConstNode(int id) {
    super(id);
  }
  ...
  public void interpret( Long id)
  {
    ...
    if( child instanceof ASTIntConstNode)
      val = ((ASTIntConstNode)child).val;
    else if( child instanceof ASTFloatConstNode)
      val = ((ASTFloatConstNode)child).val;
    ...
    if( !(node instanceof ASTMethodCallStatement) && node.jjtGetChild(0) instanceof ASTId)
    {
      ...
      ProcessStore.getInstance(id).getStack().push( value.mult(new Long(-1)).getValue());
    }
    else if( !(node instanceof ASTMethodCallStatement) && node.jjtGetChild(0) instanceof ASTArrayId)
    {
      ...
      if( primitive != null) {
        ...
        ProcessStore.getInstance(id).getStack().push( primitive.mult(new Long(-1)).getValue());
      }
      else
    ...
    else
    {
      Primitive primitive = null;
      if( child instanceof ASTIntConstNode)
        primitive = new Primitive( long.class);
      else if( child instanceof ASTFloatConstNode)
        primitive = new Primitive( double.class);
