...
class RootExecutionContext extends AbstractExecutionContext {
	...
	private static final int MAX_EXECUTION_LOG_FILE_SIZE = 10485760; // in Bytes
																		// = 10
																		// MB
	...
	public RootExecutionContext(Execution execution) {
		...
		this.executionInfo = new ExecutionInfoDTO(getExecution()
				.getExecutable().getIdentifier(), getExecution()
				.getExecutionIdentifier(), user);
	...
	public ExecutionInfoDTO getExecutionInfo() {
		...
		Set<String> keys = getKeyValuePairs().keySet();
	...
	private String createSummaryLog() {
		String statistics = getExecution().getAppJobExecutionStatistics()
	...
	public ExecutionLogDTO createLogFile() {
		...
		if (log.getTextualErrors().length() == 0
				&& log.getTextualWarnings().length() == 0) {
			return null;
		}
		...
		ExecutionLogDTO executionLogFile = executionLogDAO
				.findExecutionLogByIdentifier(executionInfo);
		...
		LOG.debug("Execution log file creation! Log string length is: "
				+ fullLOG.getBytes().length);
	...
	public void executionCanceled() {
		executionInfo
				.setExecutionStatusCode(ExecutionStatusCode.WAITING_FOR_CANCEL);
		executionInfo.setTerminationCode(ExecutionTerminationCode.CANCELED);
