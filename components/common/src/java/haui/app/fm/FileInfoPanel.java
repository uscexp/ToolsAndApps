...
import haui.io.FileInterface.vfs.FtpConnectionObject.Protokoll;
...
public class FileInfoPanel extends JPanel {
    ...
    public FileInfoPanel(JShell jshell, boolean blToolBarLeft, AppProperties appProps) {
        try {
            ...
            for (int i = 0; i < MAXBUTTONCOLUMNS * MAXBUTTONROWS; i++) {
                FileInterfaceConfiguration fic = FileConnector.createFileInterfaceConfiguration(null, 0, null, m_connObj, 0, 0, FileManager.APPNAME, m_appProps, true);
                m_buttonCmd.add(new CommandClass((new Integer(i)).toString(), "", "", "", -1, CommandClass.NO, true, this, fic));
            }
    ...
    public void updateInfo() {
        long[] lDirCombi = FileConnector.lengthAndCountOfFilesOfDir(m_fit.getFileInterface(), m_fit.getFileInterfaceFilter(), false);
        long[] lSelCombi = FileConnector.lengthAndCountOfFiles(m_fit.getSelectedFileInterfaces(), m_fit.getFileInterfaceFilter(), false);
        ...
        if (strId.equals(FileInterface.LOCAL)) {
            ...
            if (fipProps.getProperty(FileInfoPanel.FTPURL) != null) {
                ...
                try {
                    ...
                    m_jshell.getShellEnv().getErr().println(e.getMessage());
                }
                if (user == null || pwd == null) {
                    ...
                    AuthorizationDialog dlg = new AuthorizationDialog(null, "Ftp authorization", true, tmpProps, AuthorizationDialog.URLAUTH, FileManager.APPNAME);
                    dlg.setFrame(this);
                    dlg.setVisible(true);
                ...
                FtpConnectionObject f = new FtpConnectionObject(ftpUrl.getHost(), 21, user, pwd, Protokoll.FTP);
                m_connObj = f;
                // blLocal = false;
            ...
            // }
            if (m_fipTarget.getFileInfoTable().getFileInterface() != null && m_fipTarget.getFileInfoTable().getFileInterface().getId().equals(strId) && !m_blTreatAsDiff /*
                                                                                                                                                                          * &&
                                                                                                                                                                          * m_fit
                                                                                                                                                                          * .
                                                                                                                                                                          * getFileInterface
            ...
            if (cmd.equals(".."))
                ...
                for (i = 0; i < m_buttonCmd.size(); i++) {
                    if (m_buttonCmd.get(i) == null)
                        break;
                    if (cmd.equals(((CommandClass) m_buttonCmd.get(i)).getIdentifier())) {
                        String strDest = null;
                        ...
                        _exec(m_fit.getSelectedFileInterface(), strDest, (CommandClass) m_buttonCmd.get(i));
                        // blSuccess = true;
                        break;
                    }
                }
    ...
    protected void onTerminal() {
        ...
        boolean blProxyEnable = m_appProps.getProperty(GlobalApplicationContext.PROXYENABLE, false);
        Integer iProxyPort = m_appProps.getProperty(GlobalApplicationContext.PROXYPORT, 0);
        ...
        m_fipProps.setProperty(GlobalApplicationContext.PROXYENABLE, blProxyEnable);
        if (iProxyPort != null)
            m_fipProps.setProperty(GlobalApplicationContext.PROXYPORT, iProxyPort);
        ...
        if (m_jButtonCurrent != null) {
            for (int i = 0; i < MAXBUTTONCOLUMNS * MAXBUTTONROWS; i++) {
                if (m_jButtonCurrent.equals(m_jButtonCmds[i])) {
                    ...
                    if (cmd == null) {
                        FileInterfaceConfiguration fic = FileConnector.createFileInterfaceConfiguration(null, 0, null, m_connObj, 0, 0, FileManager.APPNAME, m_appProps, true);
                        cmd = new CommandClass("New", "", "New command", "", -1, CommandClass.NO, true, this, fic);
                    }
            ...
            for (int i = 0; i < MAXBUTTONCOLUMNS * MAXBUTTONROWS; i++) {
                if (m_jButtonCurrent.equals(m_jButtonCmds[i])) {
                    ...
                    try {
                        ...
                        if (str != null) {
                            StringBufferInputStream sbis = new StringBufferInputStream(str, "ISO-8859-1");
            ...
            for (int i = 0; i < MAXBUTTONCOLUMNS * MAXBUTTONROWS; i++) {
                if (m_jButtonCurrent.equals(m_jButtonCmds[i])) {
                    ...
                    if (cmd == null)
                        return;
                    FileInterfaceConfiguration fic = FileConnector.createFileInterfaceConfiguration(null, 0, null, m_connObj, 0, 0, FileManager.APPNAME, m_appProps, true);
                    setButtonCommand(new CommandClass((new Integer(i)).toString(), "", "", "", -1, CommandClass.NO, true, this, fic), i, true);
                    break;
                }
            }
        }
    ...
    public boolean _copy(boolean blWait, boolean blDelete) {
        ...
        if (fisOrg != null) {
            ...
            final CancelProgressDialog cpdlg = new CancelProgressDialog(this, strbufMsg.toString(), null, true, FileManager.APPNAME);
                ...
                public void run() {
                    ...
                    for (int i = 0; i < fisOrg.length; ++i) {
                        ...
                        if (m_blAutoLogon && m_fit.getConnectionObject() != null)
                            m_fit.getFileInterface().logon();
                        FileInterfaceConfiguration fic = FileConnector.createFileInterfaceConfiguration(null, 0, null, m_fipTarget.getFileInfoTable().getConnectionObject(), 0, 0, FileManager.APPNAME,
                                m_fipTarget.getFileInfoTable().getFileInterface().getAppProperties(), m_fipTarget.getFileInfoTable().getFileInterface().isCached());
    ...
    boolean copy(FileInterface fiOrg, FileInterface fiDest, CancelProgressDialog cpd, boolean blSetOrgDate) {
        ...
        if (m_blAutoLogon && fiOrg.getConnObj() != null)
            m_fit.getFileInterface().logon();
        if (m_blAutoLogon && fiDest.getConnObj() != null && !fiOrg.getId().equals(fiDest.getId()))
            m_fipTarget.getFileInfoTable().getFileInterface().logon();
        FileInterfaceConfiguration fic = FileConnector.createFileInterfaceConfiguration(null, 0, null, fiDest.getConnObj(), 0, 0, FileManager.APPNAME, fiDest.getAppProperties(), fiDest.isCached());
    ...
    public void _save() {
        ...
        if (strName == null) {
            ...
            if (f.exists()) {
                int iRet;
                iRet = JOptionPane.showConfirmDialog(this, "Overwrite " + strName + "?", "Confirmation", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
    ...
    public void synchronizeFileExt(List<CommandClass> cmd) {
        if ((m_fit.getFileInterface().getId().equals(m_fipTarget.getFileInfoTable().getFileInterface().getId()) && !m_blTreatAsDiff))
    ...
    void delete_actionPerformed(ActionEvent e, boolean blForce) {
        ...
        if (!blForce)
            iRet = JOptionPane.showConfirmDialog(this, "Delete all selected files?", "Confirmation", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
    ...
    public boolean configAllFileExtCommands() {
        ...
        if (idx != -1)
            strConnId = strConnId.substring(0, idx);
        FipAllPropertiesDialog dlg = new FipAllPropertiesDialog(this, "Connection properties", true, strConnId);
        dlg.setVisible(true);
        if (!dlg.isCanceled()) {
            blRet = true;
            dlg._save();
    ...
    public boolean configFileExtCommands() {
        boolean blRet = false;
        _eraseFileExt();
        FipPropertiesDialog dlg = new FipPropertiesDialog(this, "Connection properties", true, m_fipProps, m_fileCmd);
        // ConfigFileExtCommandDialog dlg = new ConfigFileExtCommandDialog(
        // getRootPane(), m_fileCmd);
        if (m_fit.getConnectionObject() != null || m_fit.getConnectionObject() instanceof FtpTypeFile || m_fit.getConnectionObject() instanceof CgiTypeFile)
            dlg.showFipPanel();
        else
            dlg.hideFipPanel();
        dlg.hideForFileEx();
