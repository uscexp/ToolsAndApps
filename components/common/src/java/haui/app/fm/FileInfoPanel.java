...
import haui.io.StringBufferInputStream;
...
/**
 * Module: FileInfoPanel.java<br>
 * $Source:
 * M:\\Dev\\source\\RCS\\M\\Dev\\source\\Bean\\ToolKit2\\haui\\app\\fm\\
 * FileInfoPanel.java,v $
 * <p>
 * Description: JPanel which contains a FileInfoTable with various buttons.<br>
 * </p>
 * <p>
 * Created: 30.10.2000 by AE
 * </p>
 * <p>
 * 
 * @history 30.10.2000 by AE: Created.<br>
 *          </p>
 *          <p>
 *          Modification:<br>
 *          $Log: FileInfoPanel.java,v $ Revision 1.4 2004-08-31 16:03:00+02
 ...
 *          features for jDirWork. Revision 1.3 2004-06-22 14:08:48+02 t026843
 *          bigger changes Revision 1.2 2003-06-06 10:04:00+02 t026843
 *          modifications because of the moving the 'TypeFile's to haui.io
 *          package Revision 1.1 2003-05-28 14:19:46+02 t026843 reorganisations
 *          Revision 1.14 2002-09-18 11:16:17+02 t026843 - changes to fit
 ...
 *          filemanager.pl Revision 1.13 2002-09-03 17:07:55+02 t026843 -
 *          CgiTypeFile is now full functional. - Migrated to the extended
 *          filemanager.pl script. Revision 1.12 2002-08-28 14:22:44+02 t026843
 *          - filmanager.pl upload added. - first preparations for the
 *          CgiTypeFile, which will repleace the FileInfo, cgi part. Revision
 *          1.11 2002-08-07 15:25:24+02 t026843 Ftp support via filetype added.
 *          Some bugfixes. Revision 1.10 2002-07-03 14:14:44+02 t026843 File
 *          context menu added Revision 1.9 2002-07-01 16:01:20+02 t026843 _exec
 *          bugfix Revision 1.8 2002-06-27 16:44:17+02 t026843 saves current dir
 *          and connection on exit Revision 1.7 2002-06-27 15:51:42+02 t026843
 *          sorting expanded and added as 'allways aktivated' Revision 1.6
 *          2002-06-26 12:06:45+02 t026843 History extended, simple bookmark
 *          system added. Revision 1.5 2002-06-21 11:00:18+02 t026843 Added gzip
 *          and bzip2 file support Revision 1.4 2002-06-19 16:13:53+02 t026843
 *          Zip file support; writing doesn't work yet! Revision 1.3 2002-06-17
 *          17:19:18+02 t026843 Zip and Jar filetype read only support. Revision
 *          1.2 2002-06-11 09:32:52+02 t026843 bugfixes Revision 1.1 2002-05-29
 *          11:18:15+02 t026843 Added: - starter menu - config starter dialog -
 *          file extensions configurable for right, middle and left mouse button
 *          Changed: - icons minimized - changed layout of file ex. and button
 *          cmd config dialog - output area hideable - other minor changes
 *          bugfixes: - some minor bugfixes Revision 1.0 2001-07-20 16:34:22+02
 ...
 *         </p>
 *         <p>
 * @version v1.0, 2000; $Revision: 1.4 $<br>
 *          $Header:
 *          M:\\Dev\\source\\RCS\\M\\Dev\\source\\Bean\\ToolKit2\\haui\\
 *          app\\fm\\FileInfoPanel.java,v 1.4 2004-08-31 16:03:00+02 t026843 Exp
 *          t026843 $
 *          </p>
 *          <p>
 * @since JDK1.2
...
public class FileInfoPanel extends JPanel {
	...
	List<CommandClass> m_buttonCmd = new ArrayList<>();
	List<CommandClass> m_fileCmd = new ArrayList<>();
	Object m_connObj;
	...
	JComboBox<String> m_jComboBoxRoots = new JComboBox<>();
	...
	public FileInfoPanel(JShell jshell, boolean blToolBarLeft,
		...
		try {
			...
			for (int i = 0; i < MAXBUTTONCOLUMNS * MAXBUTTONROWS; i++) {
				FileInterfaceConfiguration fic = FileConnector
						.createFileInterfaceConfiguration(null, 0, null,
								m_connObj, 0, 0, FileManager.APPNAME,
								m_appProps, true);
				m_buttonCmd.add(new CommandClass((new Integer(i)).toString(),
						"", "", "", -1, CommandClass.NO, true, this, fic));
			}
			m_fit = new FileInfoTable(m_fipProps) {
				...
				void leftMouseDoublePressed(MouseEvent event) {
					...
					if (fiSelected != null && fiSelected.isFile()) {
						CommandClass cmd = CommandClass.searchCommand(
								m_fileCmd, fiSelected.getName(),
								CommandClass.LEFT);

						if (cmd == null && !fiSelected.isArchive())
							_exec(fiSelected, m_fipTarget.getFileInfoTable()
									.getFileInterface().getAbsolutePath(), null);

						_execFileExtCommand(cmd);
					}
					if (fiSelected != null
							&& (fiSelected.isDirectory() || fiSelected
									.isArchive())) {
				...
				void middleMouseDoublePressed(MouseEvent event) {
					...
					if (fiSelected != null && fiSelected.isFile()) {
						...
						if (cmd == null)
							_exec(fiSelected, fiSelected.getAbsolutePath(),
									null);
				...
				public void root() {
					super.root();
					this.init((String) m_jComboBoxRoots.getSelectedItem(),
							null, m_fit.getConnectionObject(), false);
					// m_fit.repaint();
					updateLabels(true);
				}
	...
	private void jbInit() throws Exception {
		...
		m_jButtonRoot.setIcon(ResouceManager.getCommonImageIcon(
				FileManager.APPNAME, "slash.gif"));
		...
		m_jButtonParent.setIcon(ResouceManager.getCommonImageIcon(
				FileManager.APPNAME, "up.gif"));
	...
	public void cloneTargetFip() {
		m_blCloning = true;
		List<CommandClass> buttonCommands = new ArrayList<>();
		for (CommandClass commandClass : m_fipTarget.getButtonCommandVector()) {
			buttonCommands.add((CommandClass) commandClass.clone());
		}
		setButtonCommandVector(buttonCommands);
		List<CommandClass> fileExtCommands = new ArrayList<>();
		for (CommandClass commandClass : m_fipTarget.getFileExtCommandVector()) {
			fileExtCommands.add((CommandClass) commandClass.clone());
		}
		setFileExtCommandVector(fileExtCommands);
		...
		if (m_fipTarget.getLogOffCM() != null)
			m_connLogoff = (ConnectionManager) m_fipTarget.getLogOffCM()
					.clone();
		m_fipProps = m_fipTarget.getFipProps();
		m_fit.setFileInterface((FileInterface) m_fipTarget.getFileInfoTable()
				.getFileInterface().duplicate());
		...
		fitm.setFileInterfaceVector(m_fit.getFileInterface()._listFiles(
				m_fit.getFileInterfaceFilter()));
	...
	public void init(String strPath, String strParentPath, Object connObj,
			boolean blUpdate) {
		// if( m_blAutoLogon)
		// m_fit.getFileInterface().logon();
	...
	public void updateInfo() {
		long[] lDirCombi = FileConnector
				.lengthAndCountOfFilesOfDir(m_fit.getFileInterface(),
						m_fit.getFileInterfaceFilter(), false);
		long[] lSelCombi = FileConnector.lengthAndCountOfFiles(
				m_fit.getSelectedFileInterfaces(),
				m_fit.getFileInterfaceFilter(), false);
	...
	public void refresh() {
		init(m_fit.getAbsolutePath(), m_fit.getParentPath(),
				m_fit.getConnectionObject(), true);
	}
	...
	public void connect(String strId, String strPath) {
		...
		if (strId == null) {
			ConnectDialog dlg = new ConnectDialog(this, "Select location",
					true, null);
			dlg.setVisible(true);
		...
		if (m_fit.getFileInterface() != null) {
			...
			String strTargetId = m_fipTarget.getFileInfoTable()
					.getFileInterface().getId();
			if (!strSourceId.equals(strTargetId) || m_blTreatAsDiff)
		...
		if (strId.equals(FileInterface.LOCAL)) {
			...
			File file = new File(ConfigPathUtil.getCurrentReadPath(
					FileManager.APPNAME, strFile));
			if (file.exists()
					&& fipProps.load(ConfigPathUtil.getCurrentReadPath(
							FileManager.APPNAME, strFile))) {
				if (strPath.equals("."))
			...
			if (strId.equals(FileInterface.SOCKET)) {
				SocketConnectDialog scd = new SocketConnectDialog((Frame) null,
						"Insert socket connection parameter", true);
				scd.setFrame(this);
				...
				fipProps.setProperty(SOCKETPORT, new Integer(iPort));
			...
			if (file.exists()
				...
				if (strHost == null) {
					...
					iPort = fipProps.getProperty(SOCKETPORT, 0);
				}
			}
			m_strConnName = strFile;
			if (iPort == -1)
			...
			if (strId.equals(FileInterface.COMMPORT)) {
				CommPortConnectDialog ccd = new CommPortConnectDialog(
						(Frame) null, "Select comm port", true);
				ccd.setFrame(this);
				ccd.setVisible(true);
				if (ccd.isCanceled()) {
					ccd.dispose();
			...
			CommPortConnection cc = new CommPortConnection(FileManager.APPNAME,
					portId, iMode, iBaud);
			m_connObj = cc;
		} else {
			if (!strId.equals(NEWLOC)) {
			...
			if (fipProps.getProperty(FileInfoPanel.FTPURL) != null) {
				...
				if (url == null) {
					...
					m_jshell.getShellEnv().getOut()
							.println("Ftp url not defined!");
					strId = null;
				}
				if (user == null || pwd == null) {
					AppProperties tmpProps = new AppProperties();
					if (user != null)
						tmpProps.setProperty(GlobalApplicationContext.URLUSER,
								user);
					if (pwd != null)
						tmpProps.setProperty(
								GlobalApplicationContext.URLPASSWORD, pwd);
					AuthorizationDialog dlg = new AuthorizationDialog(null,
							"Ftp authorization", true, tmpProps,
				...
				cmFm = new ConnectionManager(
						fipProps.getProperty(FILEMANAGERCGIURL), m_appProps,
						FileManager.APPNAME);
				m_connObj = cmFm;
				// blLocal = false;
			} else {
				// System.err.println(
				// "Neither FileManager cgi url nor Ftp url defined!");
				m_jshell.getShellEnv()
						.getErr()
						.println(
								"Neither FileManager cgi url nor Ftp url defined!");
				strId = null;
			...
			// }
			if (m_fipTarget.getFileInfoTable().getFileInterface() != null
					&& m_fipTarget.getFileInfoTable().getFileInterface()
							.getId().equals(strId) && !m_blTreatAsDiff /*
																		 * &&
																		 * m_fit
																		 * .
																		 * getFileInterface
		...
		if (strPath != null) {
			...
			if (strOldConnId == null
					|| (strOldConnId != null && !strOldConnId.equals(strId))) {
				FipConnection fipCon = new FipConnection(strId + FILEEXT,
						FileManager.APPNAME, m_fipProps);
				fipCon._init();
				if (fipCon.getFileExtCommands() != null
						&& fipCon.getFileExtCommands().size() > 0)
					setFileExtCommandVector(fipCon.getFileExtCommands());
				if (fipCon.getButtonCommands() != null
						&& fipCon.getButtonCommands().size() == MAXBUTTONCOLUMNS
								* MAXBUTTONROWS)
	 ...
	 * 
	 * @param cm
	 *            ConnectionManager to read from
	 */
	 ...
	 * 
	 * @param cmd
	 *            Vector of CommandClass(es)
	 */
	public void setFileExtCommandVector(List<CommandClass> cmd) {
		m_fileCmd = cmd;
	}
	 ...
	 */
	public List<CommandClass> getFileExtCommandVector() {
		return m_fileCmd;
	}
	 ...
	 *            Vector of CommandClass(es)
	 */
	public void setButtonCommandVector(List<CommandClass> cmd) {
		m_buttonCmd = cmd;
		for (int i = 0; i < MAXBUTTONCOLUMNS * MAXBUTTONROWS; i++) {
			m_jButtonCmds[i].setText(m_buttonCmd.get(i).getIdentifier());
			m_jButtonCmds[i].setToolTipText(m_buttonCmd.get(i).toString());
			m_jButtonCmds[i].setMnemonic(m_buttonCmd.get(i).getMnemonic());
		}
	}
	 ...
	 */
	public List<CommandClass> getButtonCommandVector() {
		return m_buttonCmd;
	}
	...
	private class AdpFocus extends FocusAdapter {
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (cmd.equals(".."))
				...
				for (i = 0; i < m_buttonCmd.size(); i++) {
					if (m_buttonCmd.get(i) == null)
						break;
					if (cmd.equals(((CommandClass) m_buttonCmd.get(i))
							.getIdentifier())) {
						String strDest = null;
						if (m_fipTarget != null
								&& m_fipTarget.getFileInfoTable() != null)
							strDest = m_fipTarget.getFileInfoTable()
									.getFileInterface().getAbsolutePath();
						_exec(m_fit.getSelectedFileInterface(), strDest,
								(CommandClass) m_buttonCmd.get(i));
						// blSuccess = true;
						break;
					}
				}
	...
	protected void onTerminal() {
		String strProxyHost = m_appProps
				.getProperty(GlobalApplicationContext.PROXYHOST);
		String strProxyList = m_appProps
				.getProperty(GlobalApplicationContext.PROXYNOPROXYLIST);
		String strProxyUsr = m_appProps
				.getProperty(GlobalApplicationContext.PROXYUSER);
		String strProxyPwd = m_appProps
				.getProperty(GlobalApplicationContext.PROXYPASSWORD);
		boolean blProxyEnable = m_appProps.getProperty(
				GlobalApplicationContext.PROXYENABLE, false);
		Integer iProxyPort = m_appProps.getProperty(
				GlobalApplicationContext.PROXYPORT, 0);
		if (strProxyHost != null)
			m_fipProps.setProperty(GlobalApplicationContext.PROXYHOST,
					strProxyHost);
		if (strProxyList != null)
			m_fipProps.setProperty(GlobalApplicationContext.PROXYNOPROXYLIST,
					strProxyList);
		if (strProxyUsr != null)
			m_fipProps.setProperty(GlobalApplicationContext.PROXYUSER,
					strProxyUsr);
		if (strProxyPwd != null)
			m_fipProps.setProperty(GlobalApplicationContext.PROXYPASSWORD,
					strProxyPwd);
		m_fipProps.setProperty(GlobalApplicationContext.PROXYENABLE,
				blProxyEnable);
		if (iProxyPort != null)
			m_fipProps.setProperty(GlobalApplicationContext.PROXYPORT,
					iProxyPort);
		m_fit.getFileInterface().startTerminal();
	}

	protected void onBsh() {
		final JExDialog dlg = new JExDialog(null, "Bsh console", false,
				FileManager.APPNAME);
	...
	protected void onSplit() {
		FileSplitDialog fspDlg = new FileSplitDialog((Frame) null,
				"Split file", true, m_fit.getSelectedFileInterface());
		fspDlg.setVisible(true);
		if (m_fipTarget != null
				&& m_fipTarget.getFileInfoTable().getFileInterface()
						.getAbsolutePath()
						.equals(m_fit.getFileInterface().getAbsolutePath())) {
			m_fipTarget.refresh();
		}
		refresh();
	}
	...
	protected void onConcat() {
		FileConcatDialog fcDlg = new FileConcatDialog((Frame) null,
				"Concatenate files", true, m_fit.getSelectedFileInterfaces(),
				m_appProps);
		...
		if (m_jButtonCurrent != null) {
			for (int i = 0; i < MAXBUTTONCOLUMNS * MAXBUTTONROWS; i++) {
				if (m_jButtonCurrent.equals(m_jButtonCmds[i])) {
					CommandClass cmd = m_buttonCmd.get(i);
					if (cmd == null) {
						FileInterfaceConfiguration fic = FileConnector
								.createFileInterfaceConfiguration(null, 0,
										null, m_connObj, 0, 0,
										FileManager.APPNAME, m_appProps, true);
						cmd = new CommandClass("New", "", "New command", "",
								-1, CommandClass.NO, true, this, fic);
					}
					ConfigButtonCommandDialog dlg = new ConfigButtonCommandDialog(
							this, cmd);
					dlg.setVisible(true);
					if (dlg.getCommand() != null) {
						setButtonCommand(dlg.getCommand(), i, true);
						if (m_buttonCmd.get(i).getComponent() == null)
							m_buttonCmd.get(i).setComponent(this);
					}
					dlg.dispose();
					break;
				}
				...
				if (m_jButtonCurrent.equals(m_jButtonCmds[i])) {
					...
					FileInterfaceConfiguration fic = FileConnector
							.createFileInterfaceConfiguration(null, 0, null,
									...
									m_appProps, true);
					setButtonCommand(
							new CommandClass((new Integer(i)).toString(), "",
									"", "", -1, CommandClass.NO, true, this,
									fic), i, true);
					break;
				}
			}
	...
	public void showFileProperties() {
		FilePropertiesDialog fpd = new FilePropertiesDialog(
				m_fit.getSelectedFileInterface(), this, "File Properties",
				false);
		fpd.setVisible(true);
	}
	...
	public void showOpenWith() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			if (chooser.getSelectedFile() != null
				...
				String strTargetPath = m_fipTarget.getFileInfoTable()
						.getFileInterface().getAbsolutePath();
				...
				if (cmd == null) {
					FileInterfaceConfiguration fic = FileConnector
							.createFileInterfaceConfiguration(null, 0, null,
									m_connObj, 0, 0, FileManager.APPNAME,
									m_appProps, true);
					cmd = new CommandClass("tmp", fi.getParent(), strExec,
							"%p%n", 0, CommandClass.NO, true, null, fic);
				}
				String strCmd = cmd.getCompleteCommand(fi, strTargetPath);
				if (strCmd != null)
					// System.out.println( "starting ...: " + strCmd);
					m_jshell.getShellEnv().getOut()
							.println("starting ...: " + strCmd);
				try {
					fi.exec(strTargetPath, cmd,
							m_jshell.getShellEnv().getOut(), m_jshell
									.getShellEnv().getErr(), null);
					// fi.exec( strTargetPath, cmd, System.out, System.err,
					// null);
				} catch (Exception ex) {
					ex.printStackTrace(m_jshell.getShellEnv().getErr());
				}
	...
	public void _execFileExtCommand(CommandClass cmd) {
		if (cmd != null)
			_exec(m_fit.getSelectedFileInterface(), m_fipTarget
					.getFileInfoTable().getFileInterface().getAbsolutePath(),
					cmd);
	}
	...
	public void _exec(FileInterface fi, String strTargetPath, CommandClass cmd) {
		...
		if (cmd == null) {
			FileInterfaceConfiguration fic = FileConnector
					.createFileInterfaceConfiguration(null, 0, null, m_connObj,
							0, 0, FileManager.APPNAME, m_appProps, true);
			cmd = new CommandClass("tmp", fi.getParent(), fi.getAbsolutePath(),
					"", 0, CommandClass.NO, true, null, fic);
		}
		if (cmd.getExecLocal())
		...
		try {
			...
			if (strCmd != null) {
				if (!strCmd.endsWith("&"))
					strCmd += "&";
				// m_jshell.getShellEnv().setFileInterface(
				// m_fit.getFileInterface().getFileInterface().duplicate() );
				...
				fiDup.exec(strTargetPath, cmd, m_jshell.getShellEnv().getOut(),
	 ...
	 * copy files
	 * 
	 * @return true if the file or directory is successfully copied; false
	 *         otherwise
	 */
	public boolean _copy(boolean blWait, boolean blDelete) {
		...
		if (m_fit
				.getFileInterface()
				.getId()
				.equals(m_fipTarget.getFileInfoTable().getFileInterface()
						.getId())
				&& m_fit.getFileInterface()
						.getAbsolutePath()
						.equals(m_fipTarget.getFileInfoTable()
								.getFileInterface().getAbsolutePath())) {
			return blRet;
		}
		...
		if (fisOrg != null) {
			long[] lFiCombi = FileConnector.lengthAndCountOfFiles(fisOrg, null,
					true);
			...
			final CancelProgressDialog cpdlg = new CancelProgressDialog(this,
					strbufMsg.toString(), null, true, FileManager.APPNAME);
			...
			Thread th = new Thread("copythread") {
				public void run() {
					...
					for (int i = 0; i < fisOrg.length; ++i) {
						String strDest = m_fipTarget.getFileInfoTable()
								.getFileInterface().getAbsolutePath();
						String strNewPath = strDest;
						if (strNewPath.charAt(strNewPath.length() - 1) != m_fipTarget
								.getFileInfoTable().getFileInterface()
								.separatorChar())
							strNewPath += m_fipTarget.getFileInfoTable()
									.getFileInterface().separatorChar();
						strNewPath += fisOrg[i].getName();
						if (m_blAutoLogon
								&& m_fit.getConnectionObject() != null)
							m_fit.getFileInterface().logon();
						FileInterfaceConfiguration fic = FileConnector
								.createFileInterfaceConfiguration(null, 0,
										null, m_fipTarget.getFileInfoTable()
												.getConnectionObject(), 0, 0,
										FileManager.APPNAME, m_fipTarget
												.getFileInfoTable()
												.getFileInterface()
												.getAppProperties(),
										m_fipTarget.getFileInfoTable()
												.getFileInterface().isCached());
						boolean blExist = FileConnector.exists(strNewPath, fic);
						m_blSuccess = copy(fisOrg[i], m_fipTarget
								.getFileInfoTable().getFileInterface(), cpdlg,
								blDeleting);
						if (!m_blSuccess)
							m_jshell.getErr().println(
									"Error while copying "
											+ fisOrg[i].getAbsolutePath());
						if (blDeleting && m_blSuccess) {
							m_blSuccess = FileConnector
									.deleteRecusively(fisOrg[i]);
							if (!m_blSuccess)
								m_jshell.getErr().println(
						...
						if (m_blAutoLogon
								&& m_fit.getConnectionObject() != null)
							m_fit.getFileInterface().logon();
						boolean blExistAfter = FileConnector.exists(strNewPath,
								fic);
						// add FileInterface if not exist before
						if (blExistAfter && !blExist) {
							FileInterface fi = FileConnector
									.createFileInterface(strNewPath, strDest,
											false, fic);
							((FileInfoTableModel) (fitDest.getModel()))
									.addRow(fi);
						}
					}
					if (blDeleting
							&& fitOrg.getFileInterface().equals(
									m_fit.getFileInterface()))
						refresh();
					if (fitDest.getFileInterface().equals(
							m_fipTarget.getFileInfoTable().getFileInterface()))
						m_fipTarget.refresh();
				}
			};
	 ...
	 *         otherwise
	 */
	boolean copy(FileInterface fiOrg, FileInterface fiDest,
			CancelProgressDialog cpd, boolean blSetOrgDate) {
		boolean blRet = false;
		...
		if (strNewPath.charAt(strNewPath.length() - 1) != fiDest
				.separatorChar())
		...
		FileInterfaceConfiguration fic = FileConnector
				.createFileInterfaceConfiguration(null, 0, null,
						fiDest.getConnObj(), 0, 0, FileManager.APPNAME,
						fiDest.getAppProperties(), fiDest.isCached());
		if (fiOrg.isFile() || fiOrg.isArchive()) { // copy a file
			if (!m_blOverwriteAll && FileConnector.exists(strNewPath, fic)) {
				// file exists allready -> ask to overwrite it
				CopyFileDialog dlg = new CopyFileDialog(this, "Confirmation",
						true, "Overwrite file " + fiOrg.getName() + "?");
				dlg.setVisible(true);
				int iRes = dlg.getValue();
				if (iRes == CopyFileDialog.CANCEL_OPTION) {
					cpd.getMainProgressbar().setValue(
							cpd.getMainProgressbar().getValue() + 1);
					return blRet;
			...
			try {
				...
				BufferedOutputStream bos = fiDest
						.getBufferedOutputStream(strNewPath);
				blRet = fiOrg.copyFile(bos, cpd);
				bos.flush();
				bos.close();
				if (blSetOrgDate) {
					FileInterface fiTarget = FileConnector.createFileInterface(
							strNewPath, null, false, fic);
		...
		} else if (fiOrg.isDirectory()) { // copy dir recursively
			blRet = FileConnector.exists(strNewPath, fic);
			if (!blRet) // create the copied dir if not exists on dest
			{
				FileInterface fiNewDir = FileConnector.createFileInterface(
						strNewPath, null, false, fic);
				blRet = fiNewDir.mkdir();
			}
			if (blRet) { // dir exists
							// create FileInterface for dest dir
				FileInterface fi = FileConnector.createFileInterface(
						strNewPath, strDest, false, fic);
	...
	public boolean _move() {
		...
		FileInterface fiDest = m_fipTarget.getFileInfoTable()
				.getFileInterface();
		if (fis != null) {
			for (int i = fis.length - 1; i >= 0; --i) {
				if ((fiOrg.getId().equals(fiDest.getId()) && !m_blTreatAsDiff)) // move
																				// from
																				// local
																				// to
																				// local
																				...
																				// remote
				{
					StringBuffer strbufNewPath = new StringBuffer(
							fiDest.getAbsolutePath());
					if (strbufNewPath.charAt(strbufNewPath.length() - 1) != fiDest
							.separatorChar())
					...
					FileInterfaceConfiguration fic = FileConnector
							.createFileInterfaceConfiguration(null, 0, null,
									m_connObj, 0, 0, FileManager.APPNAME,
									m_appProps, true);
					FileInterface fNew = FileConnector.createFileInterface(
							strbufNewPath.toString(), null, false, fic);
					if (fNew != null)
			...
			if (!blRet) { // move from local to remote or remote to local
	...
	public void _save() {
		...
		if (strName == null) {
			...
			if (f.exists()) {
				int iRet;
				iRet = JOptionPane
						.showConfirmDialog(this, "Overwrite " + strName + "?",
								"Confirmation", JOptionPane.YES_NO_OPTION,
								JOptionPane.QUESTION_MESSAGE);
				if (iRet == JOptionPane.NO_OPTION) {
					strName = JOptionPane.showInputDialog(this,
							"New config file name?", "Input",
							JOptionPane.QUESTION_MESSAGE);
					if (strName == null || strName.equals("")) {
						// System.out.println( "Aborted saving config file!");
						m_jshell.getShellEnv().getOut()
								.println("Aborted saving config file!");
						return;
					}
		...
		m_fipProps.setProperty(strPrefix + FIPPATH, m_fit.getFileInterface()
				.getAbsolutePath());
		String strLoginPwd = m_fipProps
				.getProperty(GlobalApplicationContext.LOGONPASSWORD);
		...
		FipConnection fipCon = new FipConnection(strName, FileManager.APPNAME,
				m_fipProps);
		fipCon.setFileExtCommands(m_fileCmd);
		fipCon.setButtonCommands(m_buttonCmd);
		fipCon._save();
		m_fipProps
				.store(ConfigPathUtil.getCurrentSavePath(FileManager.APPNAME,
						strName), strHeader);
		if (strLoginPwd != null)
			m_fipProps.setProperty(GlobalApplicationContext.LOGONPASSWORD,
					strLoginPwd);
		if (strFtpPwd != null)
	...
	public void _erase(String strNewConnId) {
		...
		if (strNewConnId == null
				|| (strNewConnId != null && !strNewConnId.equals(strConnId))) {
			FipConnection fipCon = new FipConnection(strName,
					FileManager.APPNAME, m_fipProps);
			fipCon.setFileExtCommands(m_fileCmd);
			fipCon.setButtonCommands(m_buttonCmd);
			fipCon._erase();
			...
			for (int i = 0; i < MAXBUTTONCOLUMNS * MAXBUTTONROWS; i++) {
				...
				CommandClass cmd = new CommandClass(
						(new Integer(i)).toString(), "", "", "", -1, 0, true,
						null, fic);
	 ...
	 */
	public void synchronizeFileExt(List<CommandClass> cmd) {
		if ((m_fit
				.getFileInterface()
				.getId()
				.equals(m_fipTarget.getFileInfoTable().getFileInterface()
						.getId()) && !m_blTreatAsDiff))
	 ...
	 */
	public void synchronizeButton(List<CommandClass> cmd) {
		if ((m_fit
				.getFileInterface()
				.getId()
				.equals(m_fipTarget.getFileInfoTable().getFileInterface()
	...
	void delete_actionPerformed(ActionEvent e, boolean blForce) {
		...
		if (!blForce)
			iRet = JOptionPane.showConfirmDialog(this,
					"Delete all selected files?", "Confirmation",
	...
	void root_itemStateChanged(ItemEvent e) {
		String strPath = m_fit.getRootHist((String) m_jComboBoxRoots
				.getSelectedItem());
		if (strPath == null)
	...
	void rightMousePressed(MouseEvent event) {
		...
		m_jPopupMenuButtonContext.show(m_jButtonCurrent, event.getX(),
				event.getY());
	}
	...
	public boolean configAllFileExtCommands() {
		...
		if (idx != -1)
			strConnId = strConnId.substring(0, idx);
		FipAllPropertiesDialog dlg = new FipAllPropertiesDialog(this,
				"Connection properties", true, strConnId);
		dlg.setVisible(true);
		if (!dlg.isCanceled()) {
			blRet = true;
			dlg._save();
			/*
			 * dlg.showAfterFileEx(); m_fileCmd = dlg.getCommand(); if(
			 * dlg.getCommand() != null ) { } synchronizeFileExt( m_fileCmd );
			 */
		}
		dlg.dispose();
		return blRet;
	...
	public boolean configFileExtCommands() {
		boolean blRet = false;
		_eraseFileExt();
		FipPropertiesDialog dlg = new FipPropertiesDialog(this,
				"Connection properties", true, m_fipProps, m_fileCmd);
		// ConfigFileExtCommandDialog dlg = new ConfigFileExtCommandDialog(
		// getRootPane(), m_fileCmd);
		if (m_fit.getConnectionObject() != null
				|| m_fit.getConnectionObject() instanceof FtpTypeFile
	...
	public void readCommands() {
		...
		String strFileName = null;
		CommandClass cmd;
		List<CommandClass> buttonCmd = new ArrayList<>();
		List<CommandClass> fileCmd = new ArrayList<>();
