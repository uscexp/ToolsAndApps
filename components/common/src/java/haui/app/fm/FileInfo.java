 ...
 * </p>
 * <p>
 * Created: 20.10.2000 by AE
 * </p>
 * <p>
 * 
 * @history 20.10.2000 by AE: Created.<br>
 *          </p>
 *          <p>
 *          Modification:<br>
 *          $Log: FileInfo.java,v $ Revision 1.4 2004-08-31 16:03:22+02 t026843
 *          Large redesign for application dependent outputstreams, mainframes,
 *          AppProperties! Bugfixes to DbTreeTableView, additional features for
 *          jDirWork. Revision 1.3 2004-06-22 14:08:51+02 t026843 bigger changes
 *          Revision 1.2 2003-06-06 10:04:02+02 t026843 modifications because of
 *          the moving the 'TypeFile's to haui.io package Revision 1.1
 *          2003-05-28 14:19:55+02 t026843 reorganisations Revision 1.0
 *          2003-05-21 16:25:45+02 t026843 Initial revision Revision 1.10
 *          2002-09-18 11:16:18+02 t026843 - changes to fit extended
 ...
 *          filemanager.pl Revision 1.9 2002-09-03 17:07:59+02 t026843 -
 *          CgiTypeFile is now full functional. - Migrated to the extended
 *          filemanager.pl script. Revision 1.8 2002-08-28 14:22:40+02 t026843 -
 *          filmanager.pl upload added. - first preparations for the
 *          CgiTypeFile, which will repleace the FileInfo, cgi part. Revision
 *          1.7 2002-08-07 15:25:24+02 t026843 Ftp support via filetype added.
 *          Some bugfixes. Revision 1.6 2002-06-26 12:15:12+02 t026843 bugfix
 *          Revision 1.5 2002-06-26 12:06:45+02 t026843 History extended, simple
 *          bookmark system added. Revision 1.4 2002-06-21 11:00:16+02 t026843
 *          Added gzip and bzip2 file support Revision 1.3 2002-06-19
 *          16:13:52+02 t026843 Zip file support; writing doesn't work yet!
 *          Revision 1.2 2002-06-17 17:19:18+02 t026843 Zip and Jar filetype
 *          read only support. Revision 1.1 2002-05-29 11:18:18+02 t026843
 *          Added: - starter menu - config starter dialog - file extensions
 *          configurable for right, middle and left mouse button Changed: -
 *          icons minimized - changed layout of file ex. and button cmd config
 *          dialog - output area hideable - other minor changes bugfixes: - some
 *          minor bugfixes Revision 1.0 2001-07-20 16:32:55+02 t026843 Initial
 *          revision
 *          </p>
 *          <p>
 ...
 *          app\\fm\\FileInfo.java,v 1.4 2004-08-31 16:03:22+02 t026843 Exp
 *          t026843 $
 *          </p>
 *          <p>
 * @since JDK1.2
...
public class FileInfo {
	...
	public Object clone() {
		...
		FileInfo finfo = new FileInfo(m_strCurPath, m_strParent, m_blLocal, cm,
				m_strAppName, m_appProps);
		...
		if (isLocal()) {
			FileInterfaceConfiguration fic = FileConnector
					.createFileInterfaceConfiguration(null, 0, null, m_connObj,
							0, 0, m_strAppName, m_appProps, true);
			FileInterface fi = FileConnector.createFileInterface(m_strCurPath,
					m_strParent, false, fic);
	...
	static public String findFile(String strStartDir, String strFileName,
		...
		return FileConnector.findFile(strStartDir, strFileName, blRecursive,
				fic);
	}
	 ...
	 *            : current path
	 * @param blLocal
	 *            : true, for a local file; false, for a file access via cgi
	 */
	public FileInfo(String strCurPath, String strParentPath, boolean blLocal,
			Object connObj, String strAppName, AppProperties appProps) {
		m_appProps = appProps;
	 ...
	 * 
	 * @param iIdx
	 *            : index of the FileInfo in current path
	 */
	...
	/**
	 * Add new FileInfo to current path if the parent path (of strPath) is equal
	 * to this.getPath()
	 * 
	 * @param strDir
	...
	private void add(String strPath, String strParentPath) {
		if (m_finfoArray != null) {
			FileInfo finfo = new FileInfo(strPath, strParentPath, m_blLocal,
					m_connObj, m_strAppName, m_appProps);
	 ...
	 * 
	 * @param blInitFInfoArray
	 *            : true to initialize the FileInfo array for directorys
	 */
	public void init(boolean blInitFInfoArray) {
		FileInterfaceConfiguration fic = FileConnector
				.createFileInterfaceConfiguration(null, 0, null, m_connObj, 0,
						0, m_strAppName, m_appProps, true);
		FileInterface fi = FileConnector.createFileInterface(m_strCurPath,
				m_strParent, false, fic);
		// FileInterface fi = FileConnector.createFileInterface( m_strCurPath,
		// m_strParent, blInitFInfoArray, m_connObj, m_strAppName, m_appProps);
		if (fi == null)
			return;
		m_fCurrent = fi;
	...
	public void initFileInfoArray() {
		...
		if (isDirectory() || isArchive()) {
			...
			for (int i = 0; i < Array.getLength(files); i++) {
				m_finfoArray[i] = new FileInfo(files[i].getAbsolutePath(),
						files[i].getParent(), m_blLocal, m_connObj,
						m_strAppName, m_appProps);
				m_finfoArray[i]
						.setFileInterfaceFilter(getFileInterfaceFilter());
	 ...
	 * 
	 * @param iIdx
	 *            : index of the file in current path
	 * 
	 * @return true if the file is a directory
	 */
	 ...
	 * delete file
	 * 
	 * @return true if the file or directory is successfully deleted; false
	 *         otherwise
	 */
	public boolean delete() {
		...
		if (m_fCurrent == null) {
			FileInterfaceConfiguration fic = FileConnector
					.createFileInterfaceConfiguration(null, 0, null, m_connObj,
							0, 0, m_strAppName, m_appProps, true);
			FileInterface fFile = FileConnector.createFileInterface(
					getAbsolutePath(), getParent(), false, fic);
			if (fFile == null)
				return blRet;
			blRet = fFile.delete();
		} else {
		...
		if (!blRet)
			JOptionPane.showMessageDialog(GlobalApplicationContext.instance()
					.getRootComponent(), "Can't delete file!", "Info",
	 ...
	 *            : new file name
	 * 
	 * @return true if the file or directory is successfully renamed; false
	 *         otherwise
	 */
	public boolean renameTo(String strNew) {
		...
		FileInterfaceConfiguration fic = FileConnector
				.createFileInterfaceConfiguration(null, 0, null, m_connObj, 0,
						0, m_strAppName, m_appProps, true);
		FileInterface fNew = FileConnector.createFileInterface(strNewPath,
				getParent(), false, fic);
		if (fNew == null)
			return blRet;
		FileInterface fFile = m_fCurrent;
		if (fFile == null) {
			fFile = FileConnector.createFileInterface(getPath(), getParent(),
					false, fic);
			if (fFile == null)
				return blRet;
		}
	 ...
	 *            : new file path
	 * @param mode
	 *            : move mode e.g. LOCALTOLOCAL
	 * 
	 * @return true if the file or directory is successfully moved; false
	 *         otherwise
	 */
	...
	public boolean copy(FileInfo finfoDest, String strNewPath, int mode) {
		...
		case LOCALTOLOCAL: // copy from local to local
			try {
				BufferedOutputStream bo = finfoDest
		...
		if (!blRet)
			JOptionPane.showMessageDialog(GlobalApplicationContext.instance()
					.getRootComponent(), "Can't copy file!", "Info",
					JOptionPane.WARNING_MESSAGE);
		return blRet;
	}
	 ...
	 *            : BufferedOutputStream to destination
	 */
	boolean copyFile(BufferedOutputStream bo) throws IOException {
