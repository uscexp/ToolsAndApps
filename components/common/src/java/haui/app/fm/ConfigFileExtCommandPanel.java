...
/**
 * Module: ConfigFileExtCommandPanel.java<br>
 * $Source:
 * M:\\Dev\\source\\RCS\\M\\Dev\\source\\Bean\\ToolKit2\\haui\\app\\fm\\
 * ConfigFileExtCommandPanel.java,v $
 * <p>
 * Description: Dialog to config CommandClass file extensions.<br>
 * </p>
 * <p>
 * Created: 26.06.2001 by AE
 * </p>
 * <p>
 * 
 * @history 26.06.2001 by AE: Created.<br>
 *          </p>
 *          <p>
 *          Modification:<br>
 *          $Log: ConfigFileExtCommandPanel.java,v $ Revision 1.3 2004-08-31
 *          16:03:20+02 t026843 Large redesign for application dependent
 *          outputstreams, mainframes, AppProperties! Bugfixes to
 *          DbTreeTableView, additional features for jDirWork. Revision 1.2
 *          2004-06-22 14:08:56+02 t026843 bigger changes Revision 1.1
 *          2003-06-06 10:04:00+02 t026843 modifications because of the moving
 *          the 'TypeFile's to haui.io package Revision 1.0 2003-05-21
 *          16:25:40+02 t026843 Initial revision Revision 1.3 2002-06-26
 *          12:06:45+02 t026843 History extended, simple bookmark system added.
 *          Revision 1.2 2002-05-29 11:18:16+02 t026843 Added: - starter menu -
 ...
 *          Revision 1.1 2001-08-14 16:48:40+02 t026843 default button added
 *          Revision 1.0 2001-07-20 16:32:51+02 t026843 Initial revision
 *          </p>
 *          <p>
 ...
 *          app\\fm\\ConfigFileExtCommandPanel.java,v 1.3 2004-08-31 16:03:20+02
 *          t026843 Exp t026843 $
 *          </p>
 *          <p>
 * @since JDK1.2
...
public class ConfigFileExtCommandPanel extends JPanel {
	...
	List<CommandClass> m_cmd;
	DefaultListModel<CommandClass> m_dlm = new DefaultListModel<>();
	BorderLayout m_borderLayoutBase = new BorderLayout();
	ConfigCommandPanel m_configCommandPanel = new ConfigCommandPanel(
			FileManager.APPNAME);
	...
	JList<CommandClass> m_jListFileExtCommands = new JList<>();
	JButton m_jButtonUp = new JButton();
	JButton m_jButtonDown = new JButton();

	public ConfigFileExtCommandPanel(Component frame, List<CommandClass> cmd) {
		m_frame = frame;
		try {
			jbInit();
	...
	public void setCommandVector(List<CommandClass> cmd) {
		...
		if (m_cmd != null && m_cmd.size() > 0) {
			m_configCommandPanel.setCommand(m_cmd.get(0));
			m_configCommandPanel._init();
			for (int i = 0; i < m_cmd.size(); i++) {
				m_dlm.addElement(m_cmd.get(i));
	...
	void up_actionPerformed(ActionEvent e) {
		...
		if (idx == -1)
			return;
		CommandClass cmd = m_jListFileExtCommands.getSelectedValue();

		if (idx > 0) {
			CommandClass cmdprev = m_cmd.get(idx - 1);
			m_cmd.set(idx, cmdprev);
			m_cmd.set(idx - 1, cmd);
	...
	void down_actionPerformed(ActionEvent e) {
		...
		if (idx == -1)
			return;
		CommandClass cmd = (CommandClass) m_jListFileExtCommands
				.getSelectedValue();

		if (idx + 1 < m_cmd.size()) {
			CommandClass cmdnext = m_cmd.get(idx + 1);
			m_cmd.set(idx, cmdnext);
			m_cmd.set(idx + 1, cmd);
	...
	void m_jButtonSet_actionPerformed(ActionEvent e) {
		...
		if (iIdx != -1) {
			...
			m_cmd.set(iIdx, (CommandClass) cmd.clone());
			m_dlm.set(i, (CommandClass) cmd.clone());
		} else {
			m_cmd.add((CommandClass) cmd.clone());
			m_dlm.addElement((CommandClass) cmd.clone());
			if (m_dlm.size() > 1)
	...
	class LisListSelection implements ListSelectionListener {
		public void valueChanged(ListSelectionEvent event) {
			JList<CommandClass> object = (JList<CommandClass>) event.getSource();

			// int iIdx = object.getSelectedIndex();
			m_configCommandPanel.setCommand((CommandClass) object
					.getSelectedValue());
			m_configCommandPanel._init();
		}
	}
