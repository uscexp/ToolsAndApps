...
/**
 * Module: FileSearchPanel.java<br>
 * $Source:
 * M:\\Dev\\source\\RCS\\M\\Dev\\source\\Bean\\ToolKit2\\haui\\app\\fm\\
 * FileSearchPanel.java,v $
 * <p>
 * Description: file search panel.<br>
 * </p>
 * <p>
 * Created: 19.08.2004 by AE
 * </p>
 * <p>
 * 
 * @history 19.08.2004 AE: Created.<br>
 *          </p>
 *          <p>
 *          Modification:<br>
 *          $Log: FileSearchPanel.java,v $ Revision 1.0 2004-08-31 15:57:36+02
 ...
 *          app\\fm\\FileSearchPanel.java,v 1.0 2004-08-31 15:57:36+02 t026843
 *          Exp t026843 $
 *          </p>
 *          <p>
 * @since JDK1.2
...
public class FileSearchPanel extends JPanel {
	...
	protected List<FileInterface> m_vecResult = null;
	...
	JList<String> m_jListFound = new JList<>();
	DefaultListModel<String> m_defLM = new DefaultListModel<>();
	...
	JComboBox<String> m_jComboBoxPath = new JComboBox<>();
	JComboBox<String> m_jComboBoxTextSearch = new JComboBox<>();
	...
	protected JComboBox<String> m_jComboBoxSearch = new JComboBox<>();
	...
	public FileSearchPanel(FileInfoPanel fip, AppProperties appProps,
			String strAppName) {
		m_fip = fip;
		m_appProps = appProps;
		...
		try {
			...
			if (fip != null) {
				addItem(m_jComboBoxPath, m_fip.getFileInfoTable()
	...
	protected void addItem(JComboBox<String> jcb, Object obj, boolean blInit) {
		...
		if (!blFound) {
			if (jcb.getItemCount() == 0)
				jcb.addItem((String) obj);
			else
				jcb.insertItemAt((String) obj, 0);
			jcb.setSelectedItem(obj);
	...
	public void updateStatusSearching() {
		m_jTextFieldResult.setText(String.valueOf("Searching... "
				+ m_defLM.size())
				+ FOUNDTEXT);
	}
	...
	public void search() {
		...
		if (m_strSearchPath == null || m_strSearchPath.equals("")) {
			if (m_fip.getFileInfoTable() != null
					&& m_fip.getFileInfoTable().getFileInterface() != null
					&& m_fip.getFileInfoTable().getFileInterface()
							.getRootFileInterface() != null) {
				m_strSearchPath = m_fip.getFileInfoTable().getFileInterface()
						.getRootFileInterface().getAbsolutePath();
	...
	public boolean cancel() {
		...
		if (m_sth != null && m_sth.isAlive()) {
			blRet = true;
			int iRet = JOptionPane.showConfirmDialog(this, "Cancel search?",
	...
	protected void eraseComboHist(JComboBox<String> jcb) {
	...
	protected void storeComboHist(JComboBox<String> jcb) {
		String strPrefix = getPrefix(jcb);
		for (int i = 0; i < jcb.getItemCount(); i++) {
	...
	protected void initComboHist(JComboBox<String> jcb) {
		String strPrefix = getPrefix(jcb);
		String str;

		for (int i = 0; (str = m_appProps.getProperty(strPrefix
				+ String.valueOf(i))) != null; i++) {
			addItem(jcb, str, true);
		}
	}

	private String getPrefix(JComboBox<String> jcb) {
	...
	void leftMouseDoublePressed(MouseEvent event) {
		...
		FileInterfaceConfiguration fic = FileConnector
				.createFileInterfaceConfiguration(null, 0, null, m_fip
						.getFileInfoTable().getConnectionObject(), 0, 0,
						FileManager.APPNAME, m_fip.getFileInfoTable()
								.getFileInterface().getAppProperties(), true);
		FileInterface fi = FileConnector.createFileInterface(str, null, true,
				fic);
		if (fi.isDirectory()) {
			m_fip.init(fi);
		} else {
			m_fip.init(fi.getParent(), null, m_fip.getFileInfoTable()
					.getConnectionObject(), true);
			int iIdx = m_fip.getFileInfoTable().findFileInterfaceIndex(
					fi.getAbsolutePath());
			if (iIdx != -1)
	...
	class SearchThread extends Thread {
		...
		public void run() {
			m_blRun = true;
			try {
				m_vecResult = getFileInterfaces(m_fip.getFileInfoTable()
						.getFileInterface(), m_fip.getShell().getShellEnv(),
						m_strSearchPath, m_strFileText, m_blMarked, true);
				updateStatus();
			} catch (Exception ex) {
				ex.printStackTrace();
			}
		...
		public List<FileInterface> getFileInterfaces(FileInterface fi,
				JShellEnv jse, String strPath, String strFileText,
				boolean blMarked, boolean blStart) {
			// FileInterface fiSource = null;
			WildcardFileInterfaceFilter wfif = new WildcardFileInterfaceFilter(
					strFileText, FileManager.APPNAME);
			MultiFileInterfaceFilter mfif = new MultiFileInterfaceFilter(
					(String[]) FileConnector.getSupportedFiles().toArray(),
					FileManager.APPNAME);
			List<FileInterface> vec = new ArrayList<>();

			if (blMarked) {
				FileInterface[] fis = m_fip.getFileInfoTable()
				...
				for (int i = 0; i < fis.length && m_blRun; ++i) {
					...
					if (fint.isDirectory()
							|| (fint.isArchive() && m_blSearchArchives)) {
						List<FileInterface> vecSub = getFileInterfaces(fi, jse,
								fint.getAbsolutePath(), strFileText, false,
								false);
						vec.addAll(vecSub);
					}
					if (blStart)
				...
				vec = getSourceFileInterfaces(fi, jse, strPath, wfif, true,
						true);
				List<FileInterface> vecDirs = getSourceFileInterfaces(fi, jse,
						strPath, mfif, false, true);
				...
				for (int i = 0; i < vecDirs.size() && m_blRun; ++i) {
					FileInterface fiTmp = vecDirs.get(i);
					if (fiTmp.isDirectory()
							|| (fiTmp.isArchive() && m_blSearchArchives)) {
						List<FileInterface> vecSub = getFileInterfaces(fi, jse,
								fiTmp.getAbsolutePath(), strFileText, false,
								false);
						vec.addAll(vecSub);
					}
					if (blStart)
		...
		public List<FileInterface> getSourceFileInterfaces(FileInterface fi,
				JShellEnv jse, String strCurPath, FileInterfaceFilter fif,
				boolean blAdd, boolean blExtract) {
			List<FileInterface> vecFi = new ArrayList<>();
			...
			FileInterface fiDir = FileConnector.createFileInterface(strCurPath,
					null, blExtract, fic);
			if (fiDir != null /*
							 * && fiDir.exists() //because FtpFile always
							 * returns false
							 */
					&& (fiDir.isDirectory() || (m_blSearchArchives && fiDir
							.isArchive()))) {
			...
			if (m_blTextSearch) {
				try {
					BufferedReader br = new BufferedReader(
							new InputStreamReader(fi.getBufferedInputStream()));
					String str = null;
					int iIdx = -1;
