    ...
    throws haui.tool.shell.engine.JShellException
  {
    ...
    if( strArgs.length == 1)
    {
      ...
      { /* connect & login to host */
        ...
        f.connect( fc );
        FileInterfaceConfiguration fic = FileConnector.createFileInterfaceConfiguration( null, 0, null, f, 0, 0,
            jse.getAppName(), jse.getAppPropperties(), true);
        FileInterface fiFtp = FileConnector.createFileInterface( f.pwd(), null, false, fic);
