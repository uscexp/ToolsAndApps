 ...
 * $Source:
 * M:\\Dev\\source\\RCS\\M\\Dev\\source\\Bean\\ToolKit2\\haui\\components
 * \\ConnectionManager.java,v $
 * <p>
 ...
 *          <p>
 *          Modification:<br>
 *          $Log: ConnectionManager.java,v $ Revision 1.10 2004-08-31
 *          16:03:07+02 t026843 Large redesign for application dependent
 *          outputstreams, mainframes, AppProperties! Bugfixes to
 *          DbTreeTableView, additional features for jDirWork. Revision 1.9
 *          2003-05-28 14:19:57+02 t026843 reorganisations Revision 1.8
 *          2002-09-18 11:16:21+02 t026843 - changes to fit extended
 ...
 *          filemanager.pl Revision 1.7 2002-09-03 17:09:05+02 t026843 - Changes
 *          for the CgiTypeFile. Revision 1.6 2002-08-28 14:23:39+02 t026843 -
 *          filmanager.pl upload added. - first preparations for the
 *          CgiTypeFile, which will repleace the FileInfo, cgi part. Revision
 *          1.5 2002-04-08 15:20:08+02 t026843 Polling, Compressed, stateless-
 *          and non stateless connections. Revision 1.4 2002-01-15 11:14:24+01
 *          t026843 Data header changed to a dynamic header and extended with
 *          host and port Revision 1.3 2002-01-10 16:59:43+01 t026843 <>
 *          Revision 1.2 2001-07-20 16:29:02+02 t026843 FileManager changes
 *          Revision 1.1 2000-10-13 09:09:46+02 t026843 bugfixes + changes
 *          Revision 1.0 2000-10-05 14:48:34+02 t026843 Initial revision
 *          </p>
 *          <p>
 * @author Andreas Eisenhauer
 *         </p>
 *         <p>
 * @version v1.0, 2000; $Revision: 1.10 $<br>
 *          $Header:
 *          M:\\Dev\\source\\RCS\\M\\Dev\\source\\Bean\\ToolKit2\\haui\\
 *          components\\ConnectionManager.java,v 1.10 2004-08-31 16:03:07+02
 *          t026843 Exp t026843 $
 *          </p>
 *          <p>
 * @since JDK1.2
...
public class ConnectionManager extends Object {
	...
	// public final static String BINDATA_CONTENTTYPE =
	// "application/x-www-form-urlencoded-mixed-with-binary-data";
	...
	List<String[]> m_postParams = new ArrayList<>();
	List<String[]> m_cookies = new ArrayList<>();
	int m_iRetries;
	Component m_frame;
	String m_auth;
	String m_sessionId;
	...
	public Object clone() {
		ConnectionManager cm = new ConnectionManager(new String(
				m_url.toString()), m_appProps, m_strAppName);
		cm.setFrame(m_frame);
		return cm;
	}
	...
	public ConnectionManager(String strUrl, AppProperties appProps,
			String strAppName) {
		m_strAppName = strAppName;
		setProtocol();
		setURL(strUrl);
	...
	public ConnectionManager(URL url, String sessionId, AppProperties appProps,
			String strAppName) {
		this(url, appProps, strAppName);
		setSessionId(sessionId);
	}

	public ConnectionManager(String strUrl, String sessionId,
			AppProperties appProps, String strAppName) {
	...
	public boolean usingProxy() {
		return m_appProps.getProperty(GlobalApplicationContext.PROXYENABLE,
				false);
	}
	...
	public void connect() throws IOException {
		...
		if (usingProxy())
			m_httpConn.setRequestProperty("Proxy-Authorization",
					m_proxyHandler.getAuthorization());
	}
		...
		try {
			...
			m_httpConn
					.setRequestProperty("Content-Type",
							MULITPARTDATA_CONTENTTYPE + ";boundary="
									+ CONTENT_BOUNDARY);
			m_httpConn.setRequestProperty("User-Agent",
			...
			m_httpConn
					.setRequestProperty(
							...
							"image/gif, image/x-xbitmap,image/jpeg, image/pjpeg, application/vnd.ms-excel, "
									+ BINDATA_CONTENTTYPE
									+ ", application/msword,application/vnd.ms-powerpoint, application/pdf, application/x-comet, */*");
	...
	public BufferedOutputStream postInitial(byte[] buffer, int iLen,
			boolean sendParams) {
		try {
			m_httpConn.setDoOutput(true);
			m_httpConn.setDoInput(true);
	...
	public String getParameters() {
		...
		for (int i = 0; i < m_postParams.size(); i++) {
			if (i == 0) {
				line = URLEncoder.encode(m_postParams.get(i)[0]) + "="
						+ URLEncoder.encode(m_postParams.get(i)[1]);
			} else
				line += "&" + URLEncoder.encode(m_postParams.get(i)[0]) + "="
						+ URLEncoder.encode(m_postParams.get(i)[1]);
		}
	...
	public void transferCookies() {
		...
		for (int i = 0; i < m_cookies.size(); i++) {
			line = m_cookies.get(i)[0] + "=" + m_cookies.get(i)[1];
	...
	public String readLine() {
		...
		try {
			if (m_brd == null)
				m_brd = new BufferedReader(new InputStreamReader(
						m_httpConn.getInputStream()));
			line = m_brd.readLine();
			if (line == null) {
				m_brd.close();
				m_brd = null;
	...
	/*
	 * public String readLineFromInputStream() { String line = null; try { if(
	 * m_bis == null) m_bis = new DataInputStream( m_httpConn.getInputStream());
	 * line = m_bis.readLine(); if( line == null) { m_bis.close(); m_bis = null;
	 * m_iRetries = 0; } } catch( ProtocolException pex) {
	 * pex.printStackTrace(); } catch( IOException ioex) {
	 * ioex.printStackTrace(); disconnect(); } return line; }
	 */
	...
	class URLAuthenticator extends Authenticator {
		protected PasswordAuthentication getPasswordAuthentication() {
			...
			if (m_appProps.getProperty(GlobalApplicationContext.PROXYENABLE,
					false)
					&& strPort.equals(m_appProps
							.getProperty(GlobalApplicationContext.PROXYPORT))) {
				blURL = false;
			}
			if ((blURL
					&& (m_appProps
							.getProperty(GlobalApplicationContext.URLUSER) == null || m_appProps
							.getProperty(GlobalApplicationContext.URLPASSWORD) == null) || !blURL
					&& (m_appProps
							.getProperty(GlobalApplicationContext.PROXYUSER) == null || m_appProps
							.getProperty(GlobalApplicationContext.PROXYPASSWORD) == null))
					|| m_iRetries > 0) {
				m_authDlg = new AuthorizationDialog(null, "Authorization",
						true, m_appProps, AuthorizationDialog.URLAUTH,
						m_strAppName);
			...
			if (blURL) {
				String strPass = m_appProps
						.getProperty(GlobalApplicationContext.URLPASSWORD);
				return new PasswordAuthentication(
						m_appProps
								.getProperty(GlobalApplicationContext.URLUSER),
						(strPass != null ? strPass.toCharArray() : (new String(
								"")).toCharArray()));
			} else {
				String strPass = m_appProps
