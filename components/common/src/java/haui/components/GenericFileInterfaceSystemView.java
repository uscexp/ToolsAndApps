...
public class GenericFileInterfaceSystemView {
	...
	public static GenericFileInterfaceSystemView getFileSystemView(
	...
	public FileInterface createFileObject(FileInterface dir, String filename) {
		if (dir == null) {
			...
			return FileConnector.createFileInterface(dir, filename,
					dir.getFileInterfaceConfiguration());
		}
	}
	 ...
	 */
	public FileInterface createFileObject(String path,
			FileInterfaceConfiguration fic) {
		FileInterface f = FileConnector.createFileInterface(path, null, false,
				fic);
		if (isFileSystemRoot(f)) {
			f = createFileSystemRoot(f);
		}
	 ...
	 */
	public FileInterface createNewFolder(FileInterface containingDir)
			throws IOException {
		return FileConnector.createFileInterface(containingDir, "New Folder",
				containingDir.getFileInterfaceConfiguration());
	}

	/**
	 * Creates a new <code>FileInterface</code> object for <code>f</code> with
	 * correct behavior for a file system root directory.
	 * 
	 * @param f
	 *            a <code>FileInterface</code> object representing a file system
	...
	public FileInterface getDefaultDirectory() {
		FileInterface f = (FileInterface) ShellFolder.get(
				"fileChooserDefaultFolder", fic);
		if (isFileSystemRoot(f)) {
			f = createFileSystemRoot(f);
		}
		return f;
	 ...
	 * 
	 * @param dir
	 *            the <code>FileInterface</code> being queried
	 ...
	 * 
	 * @param f
	 *            a <code>FileInterface</code> object representing a directory
	 ...
	 * 
	 * @param f
	 *            a <code>FileInterface</code> object
	 ...
	 * 
	 * @param parent
	 *            a <code>FileInterface</code> object repesenting a directory or
	 *            special folder
	 * @param fileName
	 *            a name of a file or folder which exists in <code>parent</code>
	 * @return a FileInterface object. When parent and child are both special
	 *         folders, the <code>FileInterface</code> is a wrapper containing a
	...
	class FileSystemRoot implements FileInterface {
		...
		public boolean copyFile(BufferedOutputStream bo,
				CancelProgressDialog cpd) throws IOException {
