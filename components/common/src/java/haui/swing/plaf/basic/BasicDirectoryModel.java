/* *****************************************************************
 * Project: common
 * FileInterface:    BasicDirectoryModel.java
 * 
...
public class BasicDirectoryModel extends AbstractListModel implements PropertyChangeListener
{
  ...
  public BasicDirectoryModel(JFileInterfaceChooser filechooser)
  {
  ...
  public void propertyChange(PropertyChangeEvent e)
  {
    ...
    if(prop == JFileInterfaceChooser.DIRECTORY_CHANGED_PROPERTY || prop == JFileInterfaceChooser.FILE_VIEW_CHANGED_PROPERTY || prop == JFileInterfaceChooser.FILE_FILTER_CHANGED_PROPERTY
        || prop == JFileInterfaceChooser.FILE_HIDING_CHANGED_PROPERTY || prop == JFileInterfaceChooser.FILE_SELECTION_MODE_CHANGED_PROPERTY)
    {
      validateFileCache();
    }
  ...
  public Vector<FileInterface> getDirectories()
  {
  ...
  public Vector<FileInterface> getFiles()
  {
    synchronized(fileCache)
    {
      if(files != null)
      ...
      {
        FileInterface f = (FileInterface)fileCache.get(i);
  ...
  {
    FileInterface currentDirectory = filechooser.getCurrentDirectory();
   ...
   * 
   * @param oldFile a <code>FileInterface</code> object representing the existing file
   * @param newFile a <code>FileInterface</code> object representing the desired new file name
   * @return <code>true</code> if rename succeeded, otherwise <code>false</code>
   * @since 1.4
   */
  public boolean renameFile(FileInterface oldFile, FileInterface newFile)
  {
    synchronized(fileCache)
    {
  ...
  protected void sort(Vector<FileInterface> v)
  {
    ShellFolder.sortFiles(v);
  }

  // Obsolete - not used
  protected boolean lt(FileInterface a, FileInterface b)
  {
  ...
  {
    FileInterface currentDirectory = null;
    int fid;
    Vector runnables = new Vector(10);

    public LoadFilesThread(FileInterface currentDirectory, int fid)
    {
      super("Basic L&F FileInterface Loading Thread");
    ...
    public void run0()
    {
      GenericFileInterfaceSystemView fileSystem = filechooser.getFileSystemView();

      FileInterface[] list = fileSystem.getFiles(currentDirectory, filechooser.isFileHidingEnabled());

      Vector<FileInterface> acceptsList = new Vector<FileInterface>();
      ...
      {
        FileInterface f = (FileInterface)acceptsList.elementAt(i);
      ...
      else if(newSize < oldSize)
      {
        ...
        if(start >= 0 && end > start && fileCache.subList(end, oldSize).equals(newFileCache.subList(start, newSize)))
        {
          ...
          invokeLater(new DoChangeContents(null, 0, new Vector(fileCache.subList(start, end)), start, fid));
          newFileCache = null;
        }
      }
   ...
   * <p>
   * If <code>listener</code> is <code>null</code>, no exception is thrown and no action is
   * performed.
   * 
  ...
  /**
   * Set the busy state for the model. The model is considered busy when it is running a separate
   * (interruptable) thread in order to load the contents of a directory.
   */
