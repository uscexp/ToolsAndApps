 ...
 * @since 1.0
 */
public class TriggerDTO extends DataTransferObject implements Comparable
{
  ...
  /**
   * If this constructor is used, firing tolerance is set to 2 minutes. The firing tolerance
   * represents a range in which a job has at least ha
   * 
   * @param adminJob
   * @param executionTime
   */
  public TriggerDTO(final AdminJobDTO adminJob, Date executionTime)
  {
    ...
    int firingTolerance = GlobalApplicationContext.instance().getApplicationProperties().getProperty(TRIGGER_FIRIRING_TOLERANCE, DEFAULT_TRIGGER_FIRIRING_TOLERANCE);
   ...
   * @param executionTime
   */
  public TriggerDTO(final AdminJobDTO adminJob, Date executionTime, Date firingTolerance)
  {
    setAdminJob(adminJob);
    setExecutionTime(executionTime);
  ...
  public int compareToAllowZero(Object object)
  {
