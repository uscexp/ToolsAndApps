...
public class AppReflectionToStringBuilder extends ReflectionToStringBuilder {
	...
	private static final Log LOG = LogFactory
			.getLog(AppReflectionToStringBuilder.class);
	 ...
	 * 
	 * @param object
	 *            the Object to build a toString for, not <code>null</code>
	 */
	 ...
	 *            the Object to build a toString for, not <code>null</code>
	 * @param style
	 *            the style of the toString to create, if <code>null</code> then
	...
	static {
		try {
			...
			hibernateCollection = ReflectionUtil
					.loadClass("org.hibernate.collection.PersistentCollection");
	 ...
	 * <code>true
	 * </code>.
	 * <li><code>null</code> values are appended only if
	 * {@link #isAppendNullValues()} returns <code>false</code>.
	 * <li>Collections are only appended if they are either of type:
	 * <ul>
	 * <li><code>java.util.HashSet</code>
	 * <li>or <code>java.util.ArrayList</code>
	 * <li>or <code>
	 * org.hibernate.collection.PersistentCollection</code> AND were
	 * initialized.
	 * </ul>
	...
	protected boolean accept(Field field) {
		...
		try {
			...
			/*
			 * this can't happen. We would get a Security exception instead. In
			 * any case throw a runtime exception, if the impossible happens...
			 */
		...
		if (hibernateCollection != null
				&& ClassUtils.isAssignable(clazz, hibernateCollection)) {
			try {
				// accept field only if collection has been initialized
				Boolean initialized = (Boolean) MethodUtils.invokeMethod(value,
						"wasInitialized", null);
				return initialized.booleanValue();
			} catch (NoSuchMethodException e) {
				LOG.debug("Method 'wasInitialized()' not found. Excluding field '"
						+ field.getName() + "'.");
				return false;
			} catch (IllegalAccessException e) {
				LOG.debug("Unexpected IllegalAccessException. Excluding field '"
						+ field.getName() + "'.");
				return false;
			} catch (InvocationTargetException e) {
				LOG.debug("Unexpected InvocationTargetException. Excluding field '"
						+ field.getName() + "'.");
				return false;
			}
		}
