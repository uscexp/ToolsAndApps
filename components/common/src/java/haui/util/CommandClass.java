...
/**
 * Module: CommandClass.java<br>
 * <p>
 * Description: Hadels commands with its parameters.<br>
 * </p>
 * <p>
 * Created: 06.11.2000 by AE
 * </p>
 * <p>
 * 
 * @history 06.11.2000 by AE: Created.<br>
 *          </p>
 *          <p>
 *          Modification:<br>
 *          $Log: CommandClass.java,v $ Revision 1.2 2004-08-31 16:03:22+02
 ...
 *          bigger changes Revision 1.0 2003-06-06 10:05:38+02 t026843 Initial
 *          revision Revision 1.0 2003-05-21 16:25:38+02 t026843 Initial
 *          revision Revision 1.4 2002-09-03 17:07:57+02 t026843 - CgiTypeFile
 *          is now full functional. - Migrated to the extended filemanager.pl
 *          script. Revision 1.3 2002-08-07 15:25:23+02 t026843 Ftp support via
 *          filetype added. Some bugfixes. Revision 1.2 2002-06-17 17:19:18+02
 *          t026843 Zip and Jar filetype read only support. Revision 1.1
 *          2002-05-29 11:18:17+02 t026843 Added: - starter menu - config
 *          starter dialog - file extensions configurable for right, middle and
 *          left mouse button Changed: - icons minimized - changed layout of
 *          file ex. and button cmd config dialog - output area hideable - other
 *          minor changes bugfixes: - some minor bugfixes Revision 1.0
 *          2001-07-20 16:32:49+02 t026843 Initial revision
 *          </p>
 *          <p>
 ...
 *        </p>
 */
public class CommandClass extends Object implements Serializable {
	...
	public CommandClass(String strIdentifier, String strStartDir,
			String strCommand, String strParams, int iMnemonic,
			int iMouseButton, boolean blExecLocal, Component comp,
	...
	public String getCompleteStartDir(FileInterface fiSource, String strParent,
			String strDest) {
		String strCompSd = "";
	...
	public String getCommandWithPath(FileInterface fiSource, String strParent,
			String strDest) {
		String strCmdPath = "";
	...
	static public CommandClass searchCommand(List<CommandClass> cmds,
			String strName, int iMouseButton) {
		int i;
		...
		for (i = 0; i < cmds.size(); i++) {
			cmd = cmds.get(i);
			StringTokenizer st = new StringTokenizer(cmd.getIdentifier()
					.toLowerCase(), ",", false);

			while (st.hasMoreTokens()) {
				String strPart = st.nextToken().trim();
				if (strName.toLowerCase().endsWith(strPart)
						&& cmd.getMouseButton() == iMouseButton) {
					blFound = true;
					break;
				}
			}
	...
	public String getCompleteCommand(FileInterface fiSource, String strParent,
			String strDest) {
		String strCompCmd = "";
		...
		if (strParam == null)
			...
			strParam = " " + strParam;
		strCompCmd = getCommandWithPath(fiSource, strParent, strDest)
				+ strParam;
		return strCompCmd;
	}

	private String parse(String strToParse, FileInterface fiSource,
			String strParent, String strDest) {
		...
		if (strRet != null) {
			while ((iIdx = strRet.indexOf(SOURCEPATH)) != -1) {
				if (fiSource == null)
					...
					if (str.charAt(str.length() - 1) == sepChar)
						strRet = strRet.substring(0, iIdx) + strParent
								+ strRet.substring(iIdx + 2);
					else
						strRet = strRet.substring(0, iIdx) + strParent
								+ sepChar + strRet.substring(iIdx + 2);
				}
			}
			while ((iIdx = strRet.indexOf(SOURCENAME)) != -1) {
				if (fiSource == null)
					...
					strRet = strRet.substring(0, iIdx) + fiSource.getName()
							+ strRet.substring(iIdx + 2);
			}
			while ((iIdx = strRet.indexOf(DESTPATH)) != -1) {
				if (strDest == null)
					...
					strRet = strRet.substring(0, iIdx) + strDest
							+ strRet.substring(iIdx + 2);
			}
			while ((iIdx = strRet.indexOf(EXEPATH)) != -1) {
				String strPath = null;
				FileInterface fi = FileConnector.createFileInterface(".", null,
						false, getFileInterfaceConfiguration());
				strPath = FileConnector.findCommandPathInExtendedPath(fi,
						strRet, getFileInterfaceConfiguration());
				if (strPath == null)
					strRet = strRet.substring(0, iIdx) + ""
							+ strRet.substring(iIdx + 2);
				else
					strRet = strRet.substring(0, iIdx) + strPath
							+ strRet.substring(iIdx + 2);
			}
			while ((iIdx = strRet.indexOf(PARAMDLG)) != -1) {
				String strCmd = strRet + " - parameter?";
				String str = JOptionPane.showInputDialog(m_comp, strCmd,
						"Parameter input", JOptionPane.QUESTION_MESSAGE);
				if (str == null)
					str = "";
				strRet = strRet.substring(0, iIdx) + str
						+ strRet.substring(iIdx + 2);
			}
	...
	public boolean equals(Object obj) {
		return m_strIdentifier.equals(((CommandClass) obj).getIdentifier())
				&& m_iMouseButton == ((CommandClass) obj).getMouseButton();
	}
	...
	public String toString() {
		...
		return m_strIdentifier + ", " + strStartDir + ", " + m_strCommand + " "
				+ m_strParams + strMnemonic + strMouseButton + ", "
				+ String.valueOf(m_blExecLocal);
	}

	public Object clone() {
		CommandClass cmd = new CommandClass(m_strIdentifier, m_strStartDir,
				m_strCommand, m_strParams, m_iMnemonic, m_iMouseButton,
				m_blExecLocal, m_comp, getFileInterfaceConfiguration());
		return cmd;
	}

	static public void eraseCommands(String strPrefix, AppProperties appProps) {
		for (int i = 0; appProps.getProperty(strPrefix + String.valueOf(i)
				+ "." + IDENTIFIER) != null; i++) {
	...
	static public List<CommandClass> initCommandVector(String strPrefix,
			String strAppName, AppProperties appProps) {
		int i = 0;
		CommandClass cmd;
		String strId;
		String strSd;
		...
		boolean blExecLocal;
		List<CommandClass> vecCommands = new ArrayList<>();

		for (i = 0; (strId = appProps.getProperty(strPrefix + String.valueOf(i)
				+ "." + IDENTIFIER)) != null
				&& (strCom = appProps.getProperty(strPrefix + String.valueOf(i)
						+ "." + COMMAND)) != null
				&& (strPar = appProps.getProperty(strPrefix + String.valueOf(i)
						+ "." + PARAMETER)) != null; i++) {
			cmd = new CommandClass(strAppName);
			cmd.setIdentifier(strId);
			strSd = strId = appProps.getProperty(strPrefix + String.valueOf(i)
					+ "." + STARTDIR);
			cmd.setStartDir(strSd);
			cmd.setCommand(strCom);
			cmd.setParams(strPar);
			iMnemonic = appProps.getProperty(strPrefix + String.valueOf(i)
					+ "." + MNEMONIC, new Integer(-1));
			cmd.setMnemonic(iMnemonic.intValue());
			iMouseButton = appProps.getProperty(strPrefix + String.valueOf(i)
					+ "." + MOUSEBUTTON, new Integer(CommandClass.NO));
			cmd.setMouseButton(iMouseButton.intValue());
			blExecLocal = appProps.getProperty(strPrefix + String.valueOf(i)
					+ "." + EXECLOCAL, true);
	...
	static public void storeCommands(String strPrefix,
			List<CommandClass> commands, AppProperties appProps) {
		for (int i = 0; i < commands.size(); i++) {
			CommandClass cmd = commands.get(i);
			appProps.setProperty(strPrefix + String.valueOf(i) + "."
					+ IDENTIFIER, cmd.getIdentifier());
			appProps.setProperty(
					strPrefix + String.valueOf(i) + "." + STARTDIR,
					(cmd.getStartDir() == null ? "" : cmd.getStartDir()));
			appProps.setProperty(strPrefix + String.valueOf(i) + "." + COMMAND,
					cmd.getCommand());
			appProps.setProperty(strPrefix + String.valueOf(i) + "."
					+ PARAMETER, cmd.getParams());
			appProps.setProperty(
					strPrefix + String.valueOf(i) + "." + MNEMONIC,
					new Integer(cmd.getMnemonic()));
			appProps.setProperty(strPrefix + String.valueOf(i) + "."
					+ MOUSEBUTTON, new Integer(cmd.getMouseButton()));
			appProps.setProperty(strPrefix + String.valueOf(i) + "."
					+ EXECLOCAL, cmd.getExecLocal());
		}
	}
