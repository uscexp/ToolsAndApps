/***************************************************************************************************
 * Project: common File: File.java
 * 
 * Creation: 06.03.2006 by Andreas Eisenhauer Modification: %date_modified: % %derived_by: %
 * Version: %version: %
 * 
 ...
 * Description: File<br>
 * </p><p>
 * Created: 06.03.2006 by Andreas Eisenhauer
 * </p><p>
 * @history 06.03.2006 by AE: Created.<br>
 * </p><p>
...
public class File implements Serializable, Comparable
{
  ...
  public File( char seperatorChar, char pathSeperatorChar)
  {
    this( ".");
    File.separatorChar = seperatorChar;
    File.pathSeparatorChar = pathSeperatorChar;
  }
   ...
   * 
   * <p>
   * If <code>parent</code> is <code>null</code> then the new <code>File</code> instance is
   ...
   * <code>child</code> pathname string is absolute then it is converted into a relative pathname
   * in a system-dependent way. If <code>parent</code> is the empty string then the new
   ...
   * <code>child</code> pathname string is taken to denote either a directory or a file. If the
   * <code>child</code> pathname string is absolute then it is converted into a relative pathname
   * in a system-dependent way. If <code>parent</code> is the empty abstract pathname then the new
   * <code>File</code> instance is created by converting <code>child</code> into an abstract
   * pathname and resolving the result against a system-dependent default directory. Otherwise each
   * pathname string is converted into an abstract pathname and the child abstract pathname is
   * resolved against the parent.
   ...
   * 
   * <p>
   * The exact form of a <tt>file:</tt> URI is system-dependent, hence the transformation
   ...
   * 
   * <p>
   * The <em>parent</em> of an abstract pathname consists of the pathname's prefix, if any, and
  ...
  /**
   * Returns the abstract pathname of this abstract pathname's parent, or <code>null</code> if
   ...
   * 
   * @return <code>true</code> if this abstract pathname is absolute, <code>false</code>
   *         otherwise
   */
   ...
   * abstract pathname then the pathname string of the current user directory, which is named by the
   * system property <code>user.dir</code>, is returned. Otherwise this pathname is resolved in a
   ...
   * system-dependent way. This typically involves removing redundant names such as <tt>"."</tt>
   * and <tt>".."</tt> from the pathname, resolving symbolic links (on UNIX platforms), and
   * converting drive letters to a standard case (on Microsoft Windows platforms).
   * 
   * <p>
   ...
   * 
   * <p>
   * <b>Usage note:</b> This method does not automatically escape characters that are illegal in
   * URLs. It is recommended that new code convert an abstract pathname into a URL by first
   * converting it into a URI, via the {@link #toURI() toURI} method, and then converting the URI
   ...
   * hold when a <tt>file:</tt> URI that is created in a virtual machine on one operating system
   * is converted into an abstract pathname in a virtual machine on a different operating system.
   * 
   ...
   * virtual machine terminates. Deletion will be attempted only for normal termination of the
   ...
   * in the directory. Names denoting the directory itself and the directory's parent directory are
   * not included in the result. Each string is a file name rather than a complete path.
   * 
   * <p>
   ...
   * that of the <code>{@link #list()}</code> method, except that the strings in the returned
   * array must satisfy the filter. If the given <code>filter</code> is <code>null</code> then
   * all names are accepted. Otherwise, a name satisfies the filter if and only if the value
   * <code>true</code> results when the <code>{@link
   * FilenameFilter#accept}</code> method of the
   * filter is invoked on this abstract pathname and the name of a file or directory in the
   * directory that it denotes.
   * 
   * @param filter A filename filter
   * 
   * @return An array of strings naming the files and directories in the directory denoted by this
   *         abstract pathname that were accepted by the given <code>filter</code>. The array
   *         will be empty if the directory is empty or if no names were accepted by the filter.
   *         Returns <code>null</code> if this abstract pathname does not denote a directory, or
   ...
   * <p>
   * If this abstract pathname does not denote a directory, then this method returns
   * <code>null</code>. Otherwise an array of <code>File</code> objects is returned, one for
   * each file or directory in the directory. Pathnames denoting the directory itself and the
   * directory's parent directory are not included in the result. Each resulting abstract pathname
   * is constructed from this abstract pathname using the
   ...
   * method is the same as that of the <code>{@link #listFiles()}</code> method, except that the
   * pathnames in the returned array must satisfy the filter. If the given <code>filter</code> is
   * <code>null</code> then all pathnames are accepted. Otherwise, a pathname satisfies the filter
   * if and only if the value <code>true</code> results when the
   * <code>{@link FilenameFilter#accept}</code> method of the filter is invoked on this abstract
   ...
   * <code>null</code> then all pathnames are accepted. Otherwise, a pathname satisfies the filter
   * if and only if the value <code>true</code> results when the
   * <code>{@link FileFilter#accept(java.io.File)}</code> method of the filter is invoked on the
   * pathname.
   * 
   ...
   * @throws SecurityException If a security manager exists and its <code>{@link
   *          java.lang.SecurityManager#checkWrite(java.lang.String)}</code>
   *           method does not permit the named directory and all necessary parent directories and
   *           to be created
   */
  public boolean mkdirs()
  {
    ...
    return ( parent != null) && ( new File( parent).mkdirs() && canonFile.mkdir());
  }
   ...
   * 
   * @return <code>true</code> if and only if the renaming succeeded; <code>false</code>
   *         otherwise
   * 
   * @throws SecurityException If a security manager exists and its <code>{@link
   *          java.lang.SecurityManager#checkWrite(java.lang.String)}</code>
   ...
   * file system has a <code>root</code> directory from which all other files in that file system
   * can be reached. Windows platforms, for example, have a root directory for each active drive;
   * UNIX platforms have a single root directory, namely <code>"/"</code>. The set of available
   * filesystem roots is affected by various system-level operations such the insertion or ejection
   * of removable media and the disconnecting or unmounting of physical or virtual disk drives.
   * 
   * <p>
   * This method returns an array of <code>File</code> objects that denote the root directories of
   * the available filesystem roots. It is guaranteed that the canonical pathname of any file
   ...
   * method. Thus, for example, <code>File</code> objects denoting the root directories of the
   * mapped network drives of a Windows platform will be returned by this method, while
   * <code>File</code> objects containing UNC pathnames will not be returned by this method.
   * 
   * <p>
   ...
   * java.lang.SecurityManager#checkRead(java.lang.String)}</code>
   * method denies read access to a particular root directory, then that directory will not appear
   * in the result.
   * 
   * @return An array of <code>File</code> objects denoting the available filesystem roots, or
   *         <code>null</code> if the set of roots could not be determined. The array will be
   *         empty if there are no filesystem roots.
   * 
   * @since 1.2
   */
  ...
  private static boolean checkAndCreate( String filename, SecurityManager sm) throws IOException
  {
    if( sm != null)
    {
      ...
      {
        /*
         * Throwing the original AccessControlException could disclose the location of the default
         * temporary directory, so we re-throw a more innocuous SecurityException
         */
   ...
   * 
   * <p>
   * The <code>prefix</code> argument must be at least three characters long. It is recommended
   * that the prefix be a short, meaningful string such as <code>"hjb"</code> or
   * <code>"mail"</code>. The <code>suffix</code> argument may be <code>null</code>, in
   ...
   * truncated, but if it begins with a period character (<code>'.'</code>) then the period and
   * the first three characters following it will always be preserved. Once these adjustments have
   * been made the name of the new file will be generated by concatenating the prefix, five or more
   * internally-generated characters, and the suffix.
   * 
   * <p>
   * If the <code>directory</code> argument is <code>null</code> then the system-dependent
   * default temporary-file directory will be used. The default temporary-file directory is
   * specified by the system property <code>java.io.tmpdir</code>. On UNIX systems the default
   * value of this property is typically <code>"/tmp"</code> or <code>"/var/tmp"</code>; on
   * Microsoft Windows systems it is typically <code>"c:\\temp"</code>. A different value may be
   * given to this system property when the Java virtual machine is invoked, but programmatic
   * changes to this property are not guaranteed to have any effect upon the the temporary directory
   * used by this method.
   * 
   ...
   * 
   * @param directory The directory in which the file is to be created, or <code>null</code> if
   *          the default temporary-file directory is to be used
   * 
   ...
   * then this function behaves like <code>{@link
   * #compareTo(File)}</code>. Otherwise, it throws
   * a <code>ClassCastException</code>, since abstract pathnames can only be compared to abstract
   * pathnames.
   * 
   * @param o The <code>Object</code> to be compared to this abstract pathname
   * 
   * @return If the argument is an abstract pathname, returns zero if the argument is equal to this
   *         abstract pathname, a value less than zero if this abstract pathname is
   ...
   * 
   * @throws <code>ClassCastException</code> if the argument is not an abstract pathname
   * 
   ...
   * and only if the argument is not <code>null</code> and is an abstract pathname that denotes
   * the same file or directory as this abstract pathname. Whether or not two abstract pathnames are
   ...
   * 
   * @return <code>true</code> if and only if the objects are the same; <code>false</code>
   *         otherwise
   */
   ...
   * hash code of an abstract pathname is equal to the exclusive <em>or</em> of its pathname
   * string and the decimal value <code>1234321</code>. On Microsoft Windows systems, the hash
   * code is equal to the exclusive <em>or</em> of its pathname string, convered to lower case,
  ...
  private synchronized void writeObject( java.io.ObjectOutputStream s) throws IOException
  {
    ...
    s.writeChar( File.separatorChar); // Add the separator character
  }
